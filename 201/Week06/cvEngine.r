...
cvEngine<-function(dataf, cov.list, outcome,regfun=lm,cv.group=10,
	...
	wts=rep(1,dim(dataf)[1]),xscale=FALSE,quant=0.75,r2method='corr',
	kriging=FALSE,parlel=FALSE, nclusters=3,...)
{
...
#           print(summary(cv.pred.lm))
 ...
 ######### Cross Validated stats  ######
    ...
    outR2 <- cor(cv.pred.lm,cv.data,use='pairwise.complete.obs')^2
    if (r2method=='mse') outR2 <- max(0,1-mse(cv.pred.lm,cv.data)/var(cv.data, na.rm=TRUE), na.rm=TRUE)
    ...
    original.cv.r2 <- cor(original.cv.pred,original.data,use='pairwise.complete.obs')^2
    if (r2method=='mse') original.cv.r2 <- max(0,1-mse(original.cv.pred,original.data)/var(cv.data, na.rm=TRUE), na.rm=TRUE)
    original.cv.robr2 <- cor(original.cv.pred,original.data,method="spearman",use='pairwise.complete.obs')^2  # rank corr.
