...
## @knitr c3mathtitle1,message=FALSE,fig.width=7, fig.height=3,out.width='700px', out.height='300px',dpi=500
x=matrix(rexp(2000),nrow=20)
y=matrix(x+rexp(2000),nrow=20)
z=mapply(function(a,b) lm(b~a)$coef[2],split(x,col(x)),split(y,col(y)))
mymain=expression(paste("Distribution of ",hat(beta)," with Exponential Noise"))
mylab=paste("Least-Squared Estimate (",dim(x)[2]," runs of n=",dim(x)[1]," each)",sep="")
plot(density(z),main=mymain,xlab=mylab)
abline(v=1,col=2)
